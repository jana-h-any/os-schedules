...
class _HPFInputScreenState extends State<HPFInputScreen> {
        ...
        child: Column(
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                if (_validateInputs()) {
                  List<Map<String, dynamic>> results = Scheduler.calculateHPF(processData);

                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => ResultScreen(processes: results, algorithm: 'HPF'),
                    ),
                  );
                } else {
  ...
  bool _validateInputs() {
    for (var process in processData) {
      // Check if arrival time is a positive integer
      if (int.tryParse(process['arrivalTime']) == null || int.parse(process['arrivalTime']) < 0) {
        return false;
      }

      // Check if burst time is a positive integer and not zero
